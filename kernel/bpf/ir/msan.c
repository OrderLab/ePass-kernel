...
void msan(struct bpf_ir_env *env, struct ir_function *fun, void *param)
{
	...
	array_for(pos2, loadraw_insns)
	{
		...
		if (insn->addr_val.value.type == IR_VALUE_INSN &&
			...
			v.raw_stack = true;
			struct ir_insn *s1 = bpf_ir_create_bin_insn(
				env, insn, v, insn->addr_val.value, IR_INSN_SUB,
				IR_ALU_64, INSERT_FRONT);
			struct ir_insn *b1 = bpf_ir_create_bin_insn(
				env, s1, bpf_ir_value_insn(s1),
				bpf_ir_value_const32(8), IR_INSN_DIV, IR_ALU_64,
				INSERT_BACK);
			struct ir_insn *b2 = bpf_ir_create_bin_insn(
				env, b1, bpf_ir_value_insn(b1),
				bpf_ir_value_const32(1), IR_INSN_SUB, IR_ALU_64,
				INSERT_BACK);
			struct ir_insn *s3 = bpf_ir_create_bin_insn(
				env, b2, bpf_ir_value_insn(s1),
				bpf_ir_value_const32(8), IR_INSN_MOD, IR_ALU_64,
				INSERT_BACK);
			...
			struct ir_insn *off = bpf_ir_create_bin_insn(
				...
				IR_INSN_ADD, IR_ALU_64, INSERT_BACK);
			struct ir_insn *comp3 = bpf_ir_create_bin_insn(
				env, off, bpf_ir_value_insn(comp2),
				bpf_ir_value_insn(off), IR_INSN_RSH, IR_ALU_64,
				INSERT_BACK);
			// res = comp3 & masking
			struct ir_insn *res1 = bpf_ir_create_bin_insn(
				env, comp3, bpf_ir_value_insn(comp3),
				bpf_ir_value_const32(masking_val), IR_INSN_AND,
