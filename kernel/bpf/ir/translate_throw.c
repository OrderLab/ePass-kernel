...
void translate_throw_helper(struct bpf_ir_env *env, struct ir_function *fun)
{
	...
	array_for(pos2, throw_insns)
	{
		...
		if (env->opts.enable_throw_msg) {
			// Print some error message before return
			struct ir_insn *alloc_array =
				bpf_ir_create_allocarray_insn(env, insn,
							      IR_VR_TYPE_64, 1,
							      INSERT_FRONT);

			struct ir_insn *straw1 = bpf_ir_create_storeraw_insn(
				env, alloc_array, IR_VR_TYPE_8,
				bpf_ir_addr_val(bpf_ir_value_insn(alloc_array),
						0x4),
			...
			struct ir_insn *elemptr = bpf_ir_create_getelemptr_insn(
				env, straw2, alloc_array,
				bpf_ir_value_const32(0), INSERT_BACK);

			struct ir_insn *call_insn = bpf_ir_create_call_insn(
				env, elemptr, 6,
				INSERT_BACK); // A printk call

			bpf_ir_add_call_arg(env, call_insn,
					    bpf_ir_value_insn(elemptr));

			bpf_ir_add_call_arg(env, call_insn,
					    bpf_ir_value_const32(5));
		}
		insn->op = IR_INSN_RET;
		insn->value_num = 1;
